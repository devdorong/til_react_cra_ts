...
export const toggleTodo = (id: number): ToggleAction => ({ type: 'TOGGLE', payload: { id } });
export const deleteTodo = (id: number): DeleteAction => ({ type: 'DELETE', payload: { id } });
export const editTodo = (id: number, title: string): EditAction => ({
  type: 'EDIT',
  payload: { id, title },
});
```
